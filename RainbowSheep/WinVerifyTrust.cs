    ...
    // http://www.pinvoke.net/default.aspx/wintrust.winverifytrust
     public static class _WinVerifyTrust
     {
         ...
         public static bool checkSig(string fileName, out string errorMessage)
         {
             ...
             {
                 var trustResult = WinTrustData.WinTrust.WinVerifyTrust(
                     INVALID_HANDLE_VALUE, new Guid(WinTrustData.WinTrust.WINTRUST_ACTION_GENERIC_VERIFY_V2), wtd
                 );

                 if (trustResult == WinTrustData.WinVerifyTrustResult.Success)
                 {
                     errorMessage = null;
                     return true;
                 }
